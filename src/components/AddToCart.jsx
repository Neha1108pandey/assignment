...
import OrderConfirmationButton from './OrderConfirmationPage';
...
const AddToCart = ({ cartItems }) => {
  ...
  const handleRemoveItem = (name) => {
    setCartItemsNew((prevItems) => prevItems.filter(item => item.name !== name));
  };

  // Group items by name and calculate total price and count
  const aggregatedItems = cartItemsNew.reduce((acc, item) => {
    const existingItem = acc.find(obj => obj.name === item.name);

    if (existingItem) {
      existingItem.count += 1;
      existingItem.totalPrice += item.price; // Aggregate the price
    } else {
  ...
  const totalPrice = aggregatedItems.reduce((total, item) => total + item.totalPrice, 0);

  // Calculate total quantity of items
  const totalQuantity = aggregatedItems.reduce((total, item) => total + item.count, 0);

  return (
    <div className="mr-16  mt-24    ">
      <div className='  px-16 border-0 bg-slate-50   bg-white-50'>
      <h1 className="text-lg pt-4 font-bold mb-4 text-red-500">Your Cart ( {totalQuantity})</h1>
      {aggregatedItems.length === 0 ? (
        <div className="flex flex-col items-center text-center sm:text-left">
                      ...
                      <p className="inline-block mt-4 pb-14 text-brown-900 text-red-700 font-bold whitespace-nowrap min-[320px]:text-center">
  Your added items will appear here
        ...
        <div>
          {aggregatedItems.map((item, index) => (
            <div key={index} className="mb-14 p-4 border-b-2  rounded-lg">
              <h3 className="text-md font- font-mediu font-nor font-light">
                {item.name}
                <span>
                  <button
                    onClick={() => handleRemoveItem(item.name)}
                    className=" px-1 py-1 float-end border    rounded-full text-white hover:rounded-full border-slate-600 focus:outline-none" >
                              <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="none" viewBox="0 0 10 10"><path fill="#CAAFA7" d="M8.375 9.375 5 6 1.625 9.375l-1-1L4 5 .625 1.625l1-1L5 4 8.375.625l1 1L6 5l3.375 3.375-1 1Z"/></svg> 
                  </button>
                </span>
                </h3> 
    
<p className="text-red-600 flex flex-wrap items-center space-x-2 overflow-hidden">
  <span className="text-brown-700 truncate">{item.count}x</span>
  <span className="text-gray-500 truncate">@${item.price.toFixed(2)}</span>
  <span className="text-gray-700 truncate">${item.totalPrice.toFixed(2)}</span>
</p>
            </div>
          ))}
          <div className="mt-4 border-t pt-4">
  <h3 className="text-lg font-bold text-red-950 flex justify-between">
    ...
    <span>${totalPrice.toFixed(2)}</span>
      ...
      {aggregatedItems.length > 0 && (
        ...
        <div className=''> 
        {/* <div className='   border py-2 my-8 bg-slate-50 flex    '>
...
<div className='border py-2 my-8 bg-slate-50 flex items-center justify-center space-x-2'>
  ...
  <span className='text-xs font-semibold '>
    This is a carbon neutral delivery
        ...
        <button
            onClick={handleOpen}
           className="lg:px-16   mx-auto   py-2 mb-10 bg-red-700 text-white rounded-full hover:bg-red-900 focus:outline-none sm:text-center" >
  Confirm Order
</button>
      ...
      <OrderConfirmationButton isOpen={isOpen} handleClose={handleClose} orderDetails={aggregatedItems} />
    </div>
    </div>
  );
};
